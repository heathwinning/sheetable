...
export const onRequestPost: PagesFunction<Env, string, RequestData> = async (context) => {
  ...
  try {
    body = await context.request.json() as ImportPayload;
  } catch {
    return error('Invalid JSON body', 400);
  }

  if (!body.bookName?.trim()) return error('bookName is required');
  if (!body.tables?.length) return error('tables array is required');

  // Validate column types
  for (const table of body.tables) {
    const cols = table.schema?.columns ?? [];
    for (const col of cols) {
      if (!col.name?.trim()) return error(`column name is required in table "${table.name}"`);
      if (!VALID_TYPES.has(col.type)) return error(`invalid column type "${col.type}" in table "${table.name}"`);
    }
  }
  ...
  await db.batch([
    db.prepare('INSERT INTO books (id, name, owner_id) VALUES (?, ?, ?)')
      .bind(bookId, body.bookName.trim(), user.id),
    db.prepare('INSERT INTO book_members (book_id, user_id, role) VALUES (?, ?, ?)')
      .bind(bookId, user.id, 'owner'),
  ]);
  ...
  for (let ti = 0; ti < body.tables.length; ti++) {
    ...
    // Determine column definitions from schema if available, otherwise infer from first row
    ...
    // Create the physical table t_{id}
    ...
    // Insert rows in batches (D1 limit: 100 bound params per statement)
    if (table.rows.length > 0) {
      ...
      // Strip _resolved from rows to get raw values
      ...
      // Batch row inserts (max 100 statements per batch)
