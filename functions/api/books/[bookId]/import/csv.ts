...
export const onRequestPost: PagesFunction<Env, 'bookId', RequestData> = async (context) => {
  ...
  // Insert rows in batches (D1 batch limit) — _rowId auto-generated by SQLite
  const placeholders = headers.map(() => '?').join(', ');
  const insertSql = `INSERT INTO t_${tableId} (${headers.map(h => `"${h}"`).join(', ')}) VALUES (${placeholders})`;

  const BATCH_SIZE = 50;
  for (let i = 0; i < dataRows.length; i += BATCH_SIZE) {
    const batch = dataRows.slice(i, i + BATCH_SIZE);
    const stmts = batch.map(row => {
      const values = headers.map((_, ci) => row[ci] ?? '');
      return context.env.DB.prepare(insertSql).bind(...values);
    });
